...
class Cl_Admin
{
	...
	public function login( array $data )
	{
		...
		if( !empty( $data ) ){
			...
			$query = "SELECT user_id, name, email, created FROM admin where email = '$email' and password = '$password' ";
	...
	public function account( array $data )
	{
		if( !empty( $data ) ){
			...
			$password = md5( $password );
			$query = "UPDATE admin SET password = '$password' WHERE user_id = '$user_id'";
